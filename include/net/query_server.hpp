...
// unchanged snapshot (see pipeline::SnapshotStore::version()).
...
inline std::string list_json(const std::vector<pipeline::LocateSnapshot>& all,
    ...
    std::string out = "{\"snapshot_version\":" + std::to_string(snapshot_version) + ",\"books\":[";
...
private:
    ...
    void accept_loop() {
        while (!stop_requested_.load(std::memory_order_acquire)) {
            ...
            std::lock_guard<std::mutex> lock(conn_threads_mu_);
            conn_threads_.emplace_back([this, client_fd] { handle_connection(client_fd); });
        }
    }
    ...
    void handle_connection(int client_fd) {
        ...
        while (!stop_requested_.load(std::memory_order_acquire)) {
            ...
            if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) continue;  // timeout/signal — retry
    ...
    std::string handle_line(const std::string& line) {
        ...
        if (*cmd == "list") return query_wire::list_json(store_.read_all(), store_.version());
        ...
        if (*cmd == "quote") {
            ...
            return query_wire::quote_json(*snap, store_.version());
        }
