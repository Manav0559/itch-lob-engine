...
TEST_CASE("vwap ignores ticks after end_ts entirely") {
    ...
    // target_shares(1500) = 100 * (1500-1000)/(2000-1000) = 50, independent of
    // this tick's own size — Vwap catches up to the elapsed-time schedule,
    // it does not cap a slice to the print that triggered it (see the
    // "tracks the elapsed-time target" test above).
    CHECK(vwap.shares_sent() == 50);
